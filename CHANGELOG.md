...
## Resumen semanal + vínculo Despachos → Cierres mensuales

Estado: ✅ Completada

Contexto: Despachos y Cierres mensuales vivían como tablas totalmente desconectadas — `computeKpis()` (Dashboard) solo leía `cierres`, y el `ingresos` de cada cierre se escribía a mano, sin ninguna relación con lo que ya se había registrado en Despachos (cada uno con su propio `monto` = toneladas × precio_tonelada). Esto producía cifras que no cuadraban entre sí: un despacho de $10M sin ningún reflejo en el cierre mensual del mismo período. Pedido explícito del cliente: que el monto de los despachos SÍ se sume automáticamente al ingreso del cierre mensual, y una sección de resumen semanal de cero fricción (sin formulario propio) que muestre cómo van los despachos de la semana y cómo eso aporta al mes en curso.

Cambios:

- **`src/lib/despachosSummary.js`** (nuevo): utilidades puras, sin dependencia de React ni Supabase — `getISOWeekRange()`/`getMonthRange()` (semana lunes-domingo / mes calendario que contiene una fecha, hoy por defecto), `getMonthRangeFromMesValue()` (convierte el `'YYYY-MM'` crudo del `<input type="month">` de Cierres a un rango de fechas), y `summarizeDespachos(despachos, {start, end})` (agrega ingresos/toneladas totales y por `producto` dentro de un rango; tolera despachos sin `fecha`/`monto`/`toneladas` válidos sin romper ni sumar `NaN`, mismo criterio que `deriveRow()`). Fechas armadas con getters locales (`getFullYear`/`getMonth`/`getDate`), nunca `toISOString()`, para no correr un día por conversión a UTC según la zona horaria de quien abra la app. Verificado con casos de borde antes de integrarlo (semana que cruza fin de mes, domingo como último día de semana en vez de primero de la próxima, febrero bisiesto vs. no bisiesto, registros con campos corruptos) — no solo revisado por inspección.
- **`src/data/agendaConfig.js`**: el campo `ingresos` de `cierres` pasa de `type: 'number'` (escrito a mano) a `type: 'computed'` — se calcula sumando `despachos.monto` de todos los despachos cuya `fecha` cae dentro del `mes` elegido, vía `summarizeDespachos`/`getMonthRangeFromMesValue`. Un mes sin despachos da `$0` (válido, no bloquea guardar); `null` solo antes de elegir mes. `costos` y `despachos_cerrados` no cambian — se mantienen manuales, no hay dato equivalente en Despachos para derivarlos.
- **`src/components/dashboard/DynamicForm.jsx`**: nuevo prop `externalData`, pasado como 2do argumento a `compute()` de cualquier campo `computed` — el campo `monto` de Despachos (que solo depende de otros campos del mismo formulario) simplemente ignora el argumento extra, sin cambio de comportamiento; es lo que permite que `ingresos` (Cierres) lea Despachos sin volver la implementación de `type: 'computed'` un caso especial.
- **`src/components/dashboard/WeeklySummary.jsx`** (nuevo) + **`AgendaScreen.jsx`**: nueva vista "Resumen semanal" (ítem de nav nuevo, ícono `calendar-range` agregado a `Icon.jsx`) — sin formulario ni tabla propia en Supabase, se deriva en vivo de `dataBySection.despachos` ya cargado. Muestra despachos/toneladas/ingresos de la semana en curso, desglose por producto, y el aporte de la semana al ingreso del mes en curso (mismo cálculo que ahora usa `ingresos` en Cierres, así ambos números siempre cuadran). La condición que decide si se renderiza `SectionView` pasa de `tab !== 'dashboard'` (exclusión a mano) a `SECTIONS[tab]` (¿existe como sección CRUD real?), porque ahora hay dos tabs de solo lectura, no una.

Compatibilidad: no se tocó el esquema de Supabase — todo se calcula en el cliente a partir de columnas que ya existían desde la Iteración 5 (`despachos.monto`, `.toneladas`, `.producto`, `.fecha`). Cierres existentes conservan su `ingresos` guardado tal cual hasta que se editen y vuelvan a guardarse (ahí sí toman el valor calculado); no hubo backfill retroactivo de registros históricos — no era el pedido, y sobrescribir cifras ya guardadas sin que se pidiera es más riesgo que beneficio. Build de producción corrido y verificado en este entorno (a diferencia de la nota de la Iteración 5).
