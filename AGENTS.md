...
## Fix de pantalla en blanco en producción (Iteración 3)

- **Causa real:** `vite.config.js` tenía `REPO_NAME = 'Emcoex-Sistema-App'` (heredado del proyecto ERP-Comex original), desincronizado del nombre real del repo (`emcoex-agenda-presidencial`). El `base` mal resuelto hacía que `index.html` pidiera el bundle principal en una URL 404, así que React nunca llegaba a montarse — no era un error de render de `WelcomeTour.jsx` ni de ningún componente. Corregido: `REPO_NAME = 'emcoex-agenda-presidencial'`.
- **`<title>`/`<meta description>` en `index.html`** actualizados de "ERP-Comex — Plataforma de Comercio Exterior" a "Agenda Presidencial" (texto arrastrado del proyecto original, nunca actualizado).
- **`ErrorBoundary` global agregado** (`src/components/common/ErrorBoundary.jsx`, envolviendo `<App />` en `main.jsx`): no es la causa de este bug puntual, pero cubre el caso real de que un componente futuro rompa al montarse — muestra un mensaje legible con botón de recarga en vez de blanco silencioso, y deja el error real en consola.
- **Regla de proceso derivada de este bug:** al renombrar un repo de GitHub Pages, `REPO_NAME`/`base` en `vite.config.js` es lo primero a revisar — un mismatch ahí produce pantalla en blanco sin ningún error de React visible en el árbol (el fallo es de red/build, antes de que exista árbol).
